DROP TYPE IF EXISTS categ_supercar;
DROP TYPE IF EXISTS tipuri_produse;

CREATE TYPE categ_supercar AS ENUM('editie limitata', 'clasic', 'custom', 'exotic', 'precomanda');
...
CREATE TABLE IF NOT EXISTS supercars (
   ...
   brand branduri_supercar DEFAULT 'Ferrari',
   capacitate_cilindrica INT NOT NULL CHECK (capacitate_cilindrica>=0),
   categorie categ_supercar DEFAULT 'clasic',
   optiuni_tuning VARCHAR [], --pot sa nu fie specificare deci nu punem NOT NULL
   decapotabil BOOLEAN NOT NULL DEFAULT FALSE,
   imagine VARCHAR(300),
